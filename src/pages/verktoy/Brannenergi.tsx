...
const Brannenergi = () => {
  ...
  const addEntry = (name: string, mjPerKg: number) => {
    setEntries((prev) => [
      ...prev,
      { id: crypto.randomUUID(), name, brannenergiPerKg: mjPerKg, mengdeKg: 0 },
    ]);
  };
  ...
  const addCustomEntry = () => {
    setEntries((prev) => [
      ...prev,
      { id: crypto.randomUUID(), name: "Egendefinert", brannenergiPerKg: 0, mengdeKg: 0 },
    ]);
  };
  ...
  const totalBrannenergi = entries.reduce(
    (sum, e) => sum + e.brannenergiPerKg * e.mengdeKg,
    0
  );

  const arealNum = parseFloat(romareal);
  const spesifikkBrannenergi =
    !isNaN(arealNum) && arealNum > 0 ? totalBrannenergi / arealNum : null;
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-xl font-bold">Brannenergi</h1>
            </div>
          </div>
        </div>
      </header>
        ...
        <div className="max-w-4xl mx-auto space-y-8">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {cat.items.map((item) => (
                      <button
                        key={item.name}
                        type="button"
                        onClick={() => addEntry(item.name, item.mjPerKg)}
          ...
          <Card className="shadow-medium">
            <CardHeader>
              ...
              <CardDescription>
                Legg inn mengde for hvert materiale og romareal for å beregne total og spesifikk brannenergi.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="romareal">Romareal (m²)</Label>
                <Input
                  id="romareal"
                  type="number"
                  step="0.1"
                  ...
                  value={romareal}
                  onChange={(e) => setRomareal(e.target.value)}
                />
                <p className="text-xs text-muted-foreground">
                  Brukes for å beregne spesifikk brannenergi (MJ/m²)
                </p>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  {entries.map((entry) => (
                    ...
                    >
                      ...
                      <Input
                        type="number"
                        value={entry.brannenergiPerKg || ""}
                        onChange={(e) =>
                          updateEntry(entry.id, "brannenergiPerKg", e.target.value)
                        }
                        className="h-9 text-sm"
                      />
                      <Input
                        type="number"
                        value={entry.mengdeKg || ""}
                        onChange={(e) =>
                          updateEntry(entry.id, "mengdeKg", e.target.value)
                        }
                        className="h-9 text-sm"
                        placeholder="kg"
                      />
                ...
                <div className="space-y-4 pt-4 border-t">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <Card className="border-primary/30 bg-primary/5">
                      <CardHeader>
                        <CardTitle className="text-base">Total brannenergi</CardTitle>
                      </CardHeader>
                      <CardContent>
                        <p className="text-2xl font-bold">
                          {Math.round(totalBrannenergi)} MJ
                        </p>
                        <p className="text-sm text-muted-foreground mt-1">
                          Sum av alle materialer
                        </p>
                      </CardContent>
                    </Card>
                    <Card
                    ...
                    >
                      ...
                      <CardContent>
                        {spesifikkBrannenergi !== null ? (
                          <>
                            <p className="text-2xl font-bold">
                              {Math.round(spesifikkBrannenergi)} MJ/m²
                            </p>
                            <p className="text-sm text-muted-foreground mt-1">
                              Total / romareal ({romareal} m²)
                            </p>
                          </>
                        ) : (
                          <p className="text-sm text-muted-foreground">
                            Oppgi romareal for å beregne
                          </p>
                        )}
                      </CardContent>
                    </Card>
                  ...
                  <div className="bg-muted p-4 rounded-lg text-sm space-y-2">
                    ...
                    <ul className="list-disc list-inside space-y-1 text-muted-foreground">
                      <li>
                        Total brannenergi: Q = Σ (m<sub>i</sub> · H<sub>c,i</sub>)
                      </li>
                      <li>
                        Spesifikk brannenergi: q = Q / A<sub>rom</sub>
                      </li>
                      <li>
                        Kalorimetriske verdier fra SFPE Handbook og NS-EN 1991-1-2
                      </li>
                      <li>
                        Faktisk brannenergi kan variere med materialets tilstand, fuktighet og sammensetning
                      </li>
                    </ul>
                  </div>
                </div>
