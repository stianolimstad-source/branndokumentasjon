                    ...
                    <div className="space-y-2">
                      ...
                      <div>
                        ...
                        {(() => {
                          ...
                          // Auto-populate and update when trapperom type changes
                          const currentExpectedTekst = trapperomOriginalTekst;
                          if (trapperomOriginalTekst && (!formData.trapperomKravTekst || formData.trapperomKravTekst !== currentExpectedTekst)) {
                            // Only auto-update if the text was never manually edited (matches a previous auto-generated value)
                            const allPossibleAutoTexts = [
                              // Generate all possible auto-texts to check if current is auto-generated
                              ...[1,2,3,4,5,6].flatMap(rkVal => {
                                const trTypes = ["Tr 1", "Tr 2", "Tr 3"];
                                return trTypes.flatMap(tt => {
                                  return [true, false].map(ikkeDirekte => {
                                    const filtered = trapperomKravListe.filter((krav) => {
                                      if (krav.id === "tr_romningsvei_videre" || krav.id === "tr_mellomliggende_rom") return ikkeDirekte;
                                      if (krav.id === "tr1_dor_bruksenhet") return tt === "Tr 1";
                                      if (krav.id === "tr2_eget_rom") return tt === "Tr 2";
                                      if (krav.id === "tr3_mellomliggende") return tt === "Tr 3";
                                      if (krav.id === "tr_roykspredning") return tt === "Tr 2" || tt === "Tr 3";
                                      return true;
                                    });
                                    return filtered.map(k => k.label).join("\n\n");
                                  });
                                });
                              })
                            ];
                            const isAutoGenerated = !formData.trapperomKravTekst || allPossibleAutoTexts.includes(formData.trapperomKravTekst);
                            if (isAutoGenerated) {
                        ...
                        <ul className="ml-4 list-disc text-foreground/80 space-y-0.5">
                          ...
                          {formData.rorIsolasjonRelevant && (
                            <>
                              ...
                              <li>Inntil 20% av samlet overflate kan ha isolasjon i klasse {
