                ...
                <div key={bt.id} style={{ marginTop: idx === 0 ? 6 : 28, pageBreakInside: "avoid" }}>
                  ...
                  {(() => {
                    ...
                    const yFor = (i: number, n: number) =>
                      n <= 1
                        ? H / 2
                        : PAD_TOP + (i + 0.5) * ((H - PAD_TOP - PAD_BOT) / n);

                    const arsakY = arsaker.map((_, i) => yFor(i, nA));
                    const barrY = aiBarrierer.map((_, i) => yFor(i, nB));
                    const konsY = bt.konsekvenser.map((_, i) => yFor(i, nK));
                    const toppCy = H / 2;
                    const toppY = toppCy - topH / 2;

                    // Anti-overlap kam på topphendelse venstre/høyre kant
                    const leftAnchorCount = harBarrierer ? nB : nA;
                    const leftAnchorY = (i: number) =>
                      toppCy + (i - (leftAnchorCount - 1) / 2) * Math.min(8, (topH - 12) / Math.max(leftAnchorCount, 1));
                    const rightAnchorY = (i: number) =>
                      toppCy + (i - (nK - 1) / 2) * Math.min(8, (topH - 12) / Math.max(nK, 1));
                    ...
                    // Bygg linjer
                    const lines: { d: string; key: string; stroke?: string }[] = [];
                    if (harBarrierer) {
                      // Årsak → relevante barrierer
                      arsaker.forEach((a, ai) => {
                        const matched = aiBarrierer
                          .map((b, bi) => ({ b, bi }))
                          .filter(({ b }) => b.arsakIds.includes(a.id));
                        const targets = matched.length
                          ? matched
                          : aiBarrierer.map((b, bi) => ({ b, bi })); // fallback: knytt til alle
                        targets.forEach(({ bi }) => {
                          lines.push({
                            key: `a${ai}-b${bi}`,
                      ...
                      // Barriere → topphendelse
                      aiBarrierer.forEach((_, bi) => {
                        lines.push({
                          key: `b${bi}-t`,
                    ...
                    const colHeader = (text: string, color: string): React.CSSProperties => ({
                      fontSize: 9,
                      fontWeight: 700,
                      color,
                      margin: 0,
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ position: "absolute", left: ARSAK.x, top: 8, width: ARSAK.w, zIndex: 2 }}>
                          <p style={colHeader("Årsaker", "#1e3a5f")}>Årsaker</p>
                        </div>
                        {harBarrierer && (
                          <div style={{ position: "absolute", left: BARR.x, top: 8, width: BARR.w, zIndex: 2 }}>
                            <p style={colHeader("Felles barrierer", "#065f46")}>Felles barrierer</p>
                          </div>
                        )}
                        <div style={{ position: "absolute", left: KONS.x, top: 8, width: KONS.w, zIndex: 2 }}>
                          <p style={colHeader("Konsekvenser", "#1e3a5f")}>Konsekvenser</p>
                        </div>
